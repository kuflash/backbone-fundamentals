...
Наш Backbone ```PhotoView``` шаблон Наблюдателя чтобы 'подписаться' на изменения в Модели данного Представления  ```this.model.bind('change',...)```. Шаблоны обрабатываются методом ```render()```, но, в отличии от некоторых других реализации, взаимодействие с пользователем тоже обрабатывается в Представлении (см. ```events```).
