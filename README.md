...
Шаблонизатор Jade используется для декларативной разметки двух шаблонов нашего приложения, индексного (layout.jade) и главного контейнера для записй Todo (todo.jade). Также он используется для тегов script с шаблонами используемыми для отображения каждой новой добавленной записи Todo.

<pre>
// Интерфейс Todo приложения
...
body
  ...
  // Измените UA-XXXXX-X на ID ващего сайта.
  script
...
Альтернативная статическая версия интерфейса, которая не использует Jade, может быть реализована следующим образом. Смотрите [тут](https://github.com/addyosmani/backbone-boilerplates/blob/master/option1/public/static.html) для полной версии или упрощенную реализацию ниже.
